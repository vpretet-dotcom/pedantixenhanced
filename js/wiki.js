...
export async function fetchArticleByTitle(title) {
  const r = await fetch(`${WIKI}?action=query&format=json&titles=${encodeURIComponent(title)}&prop=extracts|info&explaintext=true&inprop=url&origin=*`);
...
export async function fetchCatMembers(cmtitle) {
  const r = await fetch(`${WIKI}?action=query&list=categorymembers&cmtitle=${encodeURIComponent(cmtitle)}&cmlimit=500&cmtype=page&cmnamespace=0&format=json&origin=*`);
...
export async function fetchMostViewed(limit) {
  try {
    const r = await fetch(`${WIKI}?action=query&list=mostviewed&mvlimit=${limit}&format=json&origin=*`);
  ...
  for (let daysAgo = 2; daysAgo <= 5; daysAgo++) {
    ...
    try {
      const r2 = await fetch(`https://wikimedia.org/api/rest_v1/metrics/pageviews/top/fr.wikipedia/all-access/${yy}/${mm}/${dd}`);
      if (!r2.ok) continue;
  ...
  for (let i = 0; i < Math.min(limit, 30); i++) {
    try {
      const r3 = await fetch(`${WIKI}?action=query&list=random&rnnamespace=0&rnlimit=20&format=json&origin=*`);
...
export async function fetchArticle() {
  ...
  for (let t = 0; t < 8; t++) {
    const r = await fetch(`${WIKI}?action=query&format=json&generator=random&grnnamespace=0&grnlimit=10&prop=extracts|info&explaintext=true&inprop=url&origin=*`);
    const d = await r.json();
